...
def arg_check(args):
    if args.cuda not in ["0", "1", "2", "3", "4", "5", "6", "7"]:
        raise ValueError("Invalid CUDA device argument. Must be one of: 0, 1, 2, 3, 4, 5, 6, 7")
    ...
    for i, data in enumerate(tqdm(dataset), start=args.skip if args.skip else 0):
        try:
            ...
            if not i % 999 and i != 0:
                torch.save(tensor, f"experimental_image_features/{args.split}/{args.split}_features_{(i+1)//1000}.pt")
                print(f"experimental_{args.split} checkpoint {(i+1)//1000} saved")
                tensor = torch.tensor([]).to(model.device)
        except Exception as e:
            with open(f"experimental_image_features/{args.split}_features_error.txt", "a") as f:
                f.write(f"{i}: {e}\n")