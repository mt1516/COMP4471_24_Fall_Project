...
for i, data in enumerate(tqdm(dataset), start=args.skip if args.skip else 0):
    try:
        ...
        label_tensor = torch.tensor([data["label"]]).to(device)
        tensor = torch.cat((tensor, label_tensor), 0)
        if not (i+1) % 1000:
            torch.save(tensor, f"image_labels/{args.split}/{args.split}_labels_{((i+1)//1000)-1}.pt")
